...
describe('Genre routes', () => {
  ...
  describe('GET /v1/genres/:genreId', () => {
    ...
    test('should return 401 error if access token is missing', async () => {
      ...
      await request(app).get(`/v1/genres/${userOne._id}`).send().expect(httpStatus.UNAUTHORIZED);
    });
  ...
  describe('PATCH /v1/genres/:genreId', () => {
    ...
    test('should return 401 error if access token is missing', async () => {
      ...
      await request(app).patch(`/v1/genres/${userOne._id}`).send(updateBody).expect(httpStatus.UNAUTHORIZED);
    });
